...
#[derive(Debug, Default, PartialEq, Clone)]
pub struct New<'a> {
    pub process: Option<mod_Model::Process<'a>>,
}
...
impl<'a> MessageRead<'a> for New<'a> {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(10) => msg.process = Some(r.read_message::<mod_Model::Process>(bytes)?),
                Ok(t) => { r.read_unknown(bytes, t)?; }
                Err(e) => return Err(e),
            }
        }
...
impl<'a> MessageRead<'a> for Model {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        let mut msg = Self::default();
        while !r.is_eof() {
            match r.next_tag(bytes) {
...
pub struct Process<'a> {
    pub id: Cow<'a, str>,
    pub state: mod_Model::mod_Process::State,
    pub progress: Option<mod_Model::mod_Process::Progress<'a>>,
    pub spaceId: Cow<'a, str>,
    pub error: Cow<'a, str>,
    pub message: mod_Model::mod_Process::OneOfmessage,
}
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(34) => msg.progress = Some(r.read_message::<mod_Model::mod_Process::Progress>(bytes)?),
                Ok(42) => msg.spaceId = r.read_string(bytes).map(Cow::Borrowed)?,
                Ok(90) => msg.error = r.read_string(bytes).map(Cow::Borrowed)?,
                Ok(50) => msg.message = mod_Model::mod_Process::OneOfmessage::dropFiles(r.read_message::<mod_Model::mod_Process::DropFiles>(bytes)?),
                Ok(58) => msg.message = mod_Model::mod_Process::OneOfmessage::import(r.read_message::<mod_Model::mod_Process::Import>(bytes)?),
                Ok(66) => msg.message = mod_Model::mod_Process::OneOfmessage::export(r.read_message::<mod_Model::mod_Process::Export>(bytes)?),
                Ok(74) => msg.message = mod_Model::mod_Process::OneOfmessage::saveFile(r.read_message::<mod_Model::mod_Process::SaveFile>(bytes)?),
                Ok(82) => msg.message = mod_Model::mod_Process::OneOfmessage::migration(r.read_message::<mod_Model::mod_Process::Migration>(bytes)?),
                Ok(98) => msg.message = mod_Model::mod_Process::OneOfmessage::preloadFile(r.read_message::<mod_Model::mod_Process::PreloadFile>(bytes)?),
                Ok(t) => { r.read_unknown(bytes, t)?; }
                Err(e) => return Err(e),
            }
        }
...
impl<'a> MessageWrite for Process<'a> {
    fn get_size(&self) -> usize {
        0
        + if self.id == "" { 0 } else { 1 + sizeof_len((&self.id).len()) }
        + if self.state == anytype::mod_Model::mod_Process::State::None { 0 } else { 1 + sizeof_varint(*(&self.state) as u64) }
        + self.progress.as_ref().map_or(0, |m| 1 + sizeof_len((m).get_size()))
        + if self.spaceId == "" { 0 } else { 1 + sizeof_len((&self.spaceId).len()) }
        + if self.error == "" { 0 } else { 1 + sizeof_len((&self.error).len()) }
        + match self.message {
            mod_Model::mod_Process::OneOfmessage::dropFiles(ref m) => 1 + sizeof_len((m).get_size()),
            mod_Model::mod_Process::OneOfmessage::import(ref m) => 1 + sizeof_len((m).get_size()),
            mod_Model::mod_Process::OneOfmessage::export(ref m) => 1 + sizeof_len((m).get_size()),
            mod_Model::mod_Process::OneOfmessage::saveFile(ref m) => 1 + sizeof_len((m).get_size()),
            mod_Model::mod_Process::OneOfmessage::migration(ref m) => 1 + sizeof_len((m).get_size()),
            mod_Model::mod_Process::OneOfmessage::preloadFile(ref m) => 1 + sizeof_len((m).get_size()),
            mod_Model::mod_Process::OneOfmessage::None => 0,
    }    }

    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        if self.id != "" { w.write_with_tag(10, |w| w.write_string(&**&self.id))?; }
        if self.state != anytype::mod_Model::mod_Process::State::None { w.write_with_tag(24, |w| w.write_enum(*&self.state as i32))?; }
        ...
        match self.message {            mod_Model::mod_Process::OneOfmessage::dropFiles(ref m) => { w.write_with_tag(50, |w| w.write_message(m))? },
            mod_Model::mod_Process::OneOfmessage::import(ref m) => { w.write_with_tag(58, |w| w.write_message(m))? },
            mod_Model::mod_Process::OneOfmessage::export(ref m) => { w.write_with_tag(66, |w| w.write_message(m))? },
            mod_Model::mod_Process::OneOfmessage::saveFile(ref m) => { w.write_with_tag(74, |w| w.write_message(m))? },
            mod_Model::mod_Process::OneOfmessage::migration(ref m) => { w.write_with_tag(82, |w| w.write_message(m))? },
            mod_Model::mod_Process::OneOfmessage::preloadFile(ref m) => { w.write_with_tag(98, |w| w.write_message(m))? },
            mod_Model::mod_Process::OneOfmessage::None => {},
    }        Ok(())
    }
}
...
pub enum OneOfmessage {
    dropFiles(mod_Model::mod_Process::DropFiles),
    import(mod_Model::mod_Process::Import),
    export(mod_Model::mod_Process::Export),
    saveFile(mod_Model::mod_Process::SaveFile),
    migration(mod_Model::mod_Process::Migration),
    preloadFile(mod_Model::mod_Process::PreloadFile),
    None,
}
