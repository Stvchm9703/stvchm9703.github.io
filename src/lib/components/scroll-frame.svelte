<script lang="ts">
    ...
    const frameMap = import.meta.glob(
        "$assets/series/index-page/w1200/*.webp",
        {
            eager: true,
            // as: "url",
            query: "?url",
    ...
    const prefetchFrameList = frameList.filter(
        ({ frameId }) =>
            frameId % prefetchFrameOffset === 0 ||
            _prefetchFrame.includes(frameId),
    );
...
<div bind:this={elementRef} class={props.class} data-frame-current={frameIndex}>
